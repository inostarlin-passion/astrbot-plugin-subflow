...
缓存默认 30 分钟才全量同步（中间只有 bot 写穿透）。两次同步之间有人**手动填表**（把某行改成已分配/填了组员），缓存仍显示未分配；此时 `/接活` 用**陈旧缓存**过闸并写库，把手填的「组员/进度」覆盖掉。修复：所有任务级状态写在 per-record 锁内**写前对目标行单条重读**（`cache.refresh_record`，顺带刷新缓存），用最新值重判，非预期态拒绝、不覆盖。

经 grilling 敲定 4 条：

1. **方向**：写前在锁内 `refresh_record` 拉最新远端态再判定（best-effort）。腾讯无服务端 CAS，残留"重读→写入"毫秒级窗口不可消除——把覆盖窗口从 ~30 分钟压到毫秒级。
2. **范围**：接活/完成/放弃/进行中 的状态闸门由读缓存（`get_record`）改为读最新（`await refresh_record`）；`/修改任务` 也写前重读刷新，但**不因冲突拒绝**（管理员强制覆盖）。
3. **接活冲突判定**：重读后 **进度=未分配 且 组员为空** 才可接；否则拒绝（"已被 X 占用，疑似手动填表，已刷新，未覆盖"），连"只填名没改状态"也保护。
4. **开关**：始终开启，不加配置项（数据安全修复，+约 20–30 读/天可忽略，D18 后更宽）。

**边界**：`refresh_record` 抛错不捕获，直接冒泡（连不上远端时本就写不进，行为一致）；单条重读只刷目标行、不动同表其它行（不抢 D17 全量同步的播报）；complete 的前置/下游探测仍读缓存本集其它行（次要、不覆盖手填，列为已知小限制）；手动**新增**整行而缓存没有的情形不在范围（靠 D17/下次同步）。与 D5（锁）、D8（写后重读）、D17（同步 diff 播报）互补。
